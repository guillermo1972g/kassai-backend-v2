...
const CONFIG = {
  INTERVAL_MS: 90 * 1000, MAX_POSITIONS: 3, STOP_LOSS_PCT: 0.20, MIN_EDGE_PCT: 0.15, CAPITAL_LIMIT: 500,
...
async function analyzeWithClaude(prices, account, markets) {
  const sys = 'You are KASS.AI, elite autonomous trading agent. Cash: $' + account.cash.toFixed(2) + '. Positions: ' + account.positionCount + '/' + CONFIG.MAX_POSITIONS + '. Mode: ' + markets.mode + '. ' + (markets.stocks ? '' : 'STOCK MARKET CLOSED - trade crypto only (BTC/USD,ETH/USD,SOL/USD,DOGE/USD). ') + 'Min edge: ' + (CONFIG.MIN_EDGE_PCT*100) + '%. Respond ONLY in valid JSON: {"recommendation":"BUY|SELL|PASS","asset":null,"qty":null,"price":null,"target":null,"stopLoss":null,"edge":0,"confidence":"HIGH|MEDIUM|LOW","reasoning":"","marketAnalysis":"","assetType":"stock|crypto"}';
...
async function executeTrade(alpaca, analysis, account) {
  ...
  if (cost > account.cash || cost > CONFIG.CAPITAL_LIMIT) return null;
  try {
    ...
    await log('INFO', 'ORDER: ' + recommendation + ' ' + qty + 'x ' + asset);
    return order;
...
function startAgent(alpaca) {
  ...
  log('INFO', 'KASS.AI v2 started - Stocks+Crypto');
  runCycle(alpaca);
