...
app.get('/health', async (req, res) => {
    ...
    res.json({ status: error ? 'ERROR' : 'OK', supabase: error ? error.message : 'Connected', anthropic: process.env.ANTHROPIC_API_KEY ? 'OK' : 'MISSING', alpaca: process.env.ALPACA_API_KEY ? 'OK' : 'MISSING', agent: getAgentStatus().status });
});
...
app.post('/agent/analyze', async (req, res) => {
    try {
          ...
          const philosophers = philosophy && Object.keys(philosophy).length > 0
            ? Object.entries(philosophy).map(([n, p]) => n + ' ' + p + '%').join(' - ')
                  : 'Sin filosofia - analisis tecnico puro';
